...
// 20181220
var hasSource = false;//判断是否有source字段
...
function _tb(id, b, _id, vCode) { //id(du):_id下面的ul,b:大json,_id:code拼接字符串,vCode:动态显示的问题;
    ...
    if (b.questionTableAnswerMap != null) { //判断json有没有传答案过来，没有则为null
        ...
        for (var i in b.questionTableAnswerMap) { //循环答案的对象
            ...
            for (var j = 0; j < b.questionTableAnswerMap[i].length; j++) {
                ...
                for(var k in b.questionTableAnswerMap[i][j]){//20181220添加source字段判断table是否能修改
                    if(k=="source"){
                        hasSource = true;
                        t_sq["source"] = b.questionTableAnswerMap[i][j][k];
                    }
                }
    ...
    $(document).unbind("click").on("click", ".show .cy span", function () {
        ...
        //20181220
        var tableChange = true;
        $(tsj[$(this).index() + 1]).each(function(){
            
            var oThis = $(this)[0];
            for(var i in oThis){
                if(i=='source'&&oThis[i]==1){
                    tableChange = false
                }
            };
        ...
        if(!tableChange){
            // isTableChange(true,oThis);
            $(tsj[$(this).index() + 1]).each(function(){
                isTableChange(true,$(this)[0]);
            })
        };
        ...
        if (num > 1) { //num大于1的话，添加删除在页签上
            ...
            for(var j in tsj){
                ...
                $(tsj[j]).each(function(){
                    var tsjL =  $(this)[0];
                    for(var k in tsjL){
                        if(k=="source"&&tsjL[k]==1){
                            isSource = true;
                        }
                    }
                })
                if(!isSource){
                    $("<img src='img/close_icon.png'/>").appendTo(id.parent().next(".show").children(".cy").children().eq(j-1));
                }
            }
        }
        ;
    ...
    function _oSave(_this, tsj, s_t, s_l) { //保存数据;_this：页面上的加减号或者span,tsj:数据;
        ...
        for (var i = 0; i < this_li.length; i++) { //循环所有的li
            if (this_li.eq(i).css("display") == "none") { //判断是否显示,把显示的数据存储
                ...
                // 20181220
                if(hasSource){
                    if(this_li.eq(i).find('.notChange').length!=0){
                        _js["source"] = 1;
                    }else{
                        _js["source"] = 0;
                    }
...
function create(c, _id, du, _once, vCode) { //创建页面函数c:问题json，_id:div的id，du:添加元素的ul，_once:只执行一次的函数是否执行（后台答案显示在页面上），vCode:显示的问题
    ...
    if (_once) { //页面进来的时候只执行一次
        ...
        //20181220
        var newC = c.questionTableAnswerMap[1]; 
        $(newC).each(function(){
            var oThis =  $(this)[0];
            for(var j in oThis){
                if(j=="source"&&oThis[j]==1){
                    isTableChange(true,oThis);
                }
            }
        })
    ...
    if (answerType == "ONLYTABLE" || answerType == "TABLE" || answerType == "MULTIPLE" || answerType == "CHECKBOXMULT" || answerType == "INPUTTABLE") {
        ...
        for (var i = 0; i < du.parent().siblings(".show").children("ul").children().length; i++) {
            if (du.parent().siblings(".show").children("ul").children().eq(i).css("display") == "none") {
                ...
                // 20181220
                if(hasSource){
                    if(du.parent().siblings(".show").children("ul").children().eq(i).find('.notChange').length!=0){
                        _js["source"] = 1;
                    }else{
                        _js["source"] = 0;
                    }
...
// 20181220
function isTableChange(isChange,listSj){
    if(isChange){
        $('.show li').each(function(){
            if($(this).attr('data')==listSj.investigateTableCode){
                $(this).find('.right').append('<div class="notChange"></div>');
