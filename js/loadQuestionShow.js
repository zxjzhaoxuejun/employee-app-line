...
function loadQuestionShow(json) { //页面生成方法\
	$("<section></section>").appendTo($("body"));
	$("<div class='maskMove'></div>").appendTo($("body"));
	var data=[];
	for (var i = 0; i < json.length; i++) {
		var QuestionSortNo=json[i].sortNo;
		data[i]=$.parseJSON(json[i].modelValue);
		var QuestionName=data[i].name;
		var QuestionType=data[i].answerType;
		_data = json[i]; //把后台传过来的json赋值给_data
		var dataQuestion=data[i];
		console.log(_data)
		...
		if (json[i].questionTableAnswerMap != undefined) {
			data[i]["questionTableAnswerMap"] = json[i].questionTableAnswerMap;
		} else {
			data[i]["questionTableAnswerMap"] = null;
		};
		if (json[i].value != undefined) {
			data[i]["value"] = json[i].value;
		} else {
			data[i]["value"] = null;
		};
		console.log(data[i])
		
		if (QuestionType == "TABLE") { //TABLE类型
			du = center(dataQuestion); //执行center函数
			answerType = "TABLE";
		}else if (QuestionType == "MULTIPLE") { //MULTIPLE类型
			du = center(dataQuestion); //执行center函数
			answerType = "MULTIPLE";
		}else if (QuestionType == "ONLYTABLE") { //ONLYTABLE类型
			du = right(dataQuestion); //执行right函数
			answerType = "ONLYTABLE";
		}else if (QuestionType == "CHECKBOXMULT") { //CHECKBOXMULT类型
			du = left(dataQuestion); //执行left函数
			answerType = "CHECKBOXMULT";
			du.parent().next(".show").children(".cy").hide();
			du.parent().next(".show").children("p").first().children(".add,.jian").hide();
		}else if (QuestionType == "INPUTTABLE") { //INPUTTABLE类型
			du = inputTable(dataQuestion);
			answerType = "INPUTTABLE";
		}else if(QuestionType == "INPUT") { //INPUT类型
			du = _input(dataQuestion);
			answerType = "INPUT";
		}
//		if (dataQuestion.isEnd) { //判断是否是最后一个(后台传过来的数据)
//			$("#next p").text("确认");
//		}
//		if (QuestionType== "TABLE" || QuestionType == "ONLYTABLE" || QuestionType == "INPUTTABLE") { //table类型
...
//			if (num == dataQuestion.answerLimit) {
//				du.parent().next(".show").children("p").first().children(".add").hide();
//			}
//			//answerLimitMin如果为0可以无限加，如果不为0;就有最小值
//			if (num > 1) {
//				if (dataQuestion.answerLimitMin != 0) {
//					if (num > dataQuestion.answerLimitMin) {
...
//		var _checked = [];
//		var _chk = _data;
//		_chk["fieldName"] = dataQuestion.code;
//		_chk["inputType"] = dataQuestion.answerInput;
//		_checked.push(_chk);
//		h_zjChecked.initQuestionMap("question", _checked);
//		h_zjChecked.initAutoCheckTrigger();
//		//黄泽建校验规则调用方法结束
//		$(document).on("keyup", "input[astype='integer']", function() { //整数输入
...
function center(v) { //TABLE、Multiple类型;_id:给div设的id;v:大json;
	console.log(v)
		//刚进来的时候先创建section放到body里面
		if (v.length != 0) {
			var _idsp = v.code.split("."); //分割code
			console.log(_idsp )
			var _id = ""; //给div设id
		...
		var cp = $("<div modelname='question' class='problem' id='"+_id+"'><div class='pb'><p class='view-title01' data='"+v.code+"' onclick='bjQuestion(this)'>" + v.name + "<em>编辑</em></p></div><ul></ul></div>");
		$("section").append(cp);
	var du = $("#" + _id + " ul");
	$("#" + _id + " ul").show();
...
function right(c) { //ONLYTABLE类型;_id:给div设的id;c:大json;
	if (c.length != 0) {
		...
		var cp = $("<div modelname='question' class='problem' id='"+_id+"'><div class='pb'><p class='view-title01' data='"+c.code+"' onclick='bjQuestion(this)'>" + c.name + "<em>编辑</em></div><ul></ul></div>");
		$("section").append(cp);
...
function inputTable(v) { //INPUTTABLE类型;_id:给div设的id;v:大json;
	if (v.length != 0) {
			var _idsp = v.code.split("."); //分割code
			console.log(_idsp )
			var _id = ""; //给div设id
		...
		var cp = $("<div modelname='question' class='problem' id='"+_id+"'><div class='pb'><p class='view-title01' data='"+v.code+"' onclick='bjQuestion(this)'>" + v.name + "<em>编辑</em></div><ul></ul></div>");
		$("section").append(cp);
...
function _ipt(cqt, du, _id) { //INPUTTABLE;_id:给div设的id;cqt:大json;du:du为页面上第一个'.problem ul';
	...
	if (cqt.answerInput == "STRING") { //文本;type='text'
		_iptli = $('<li fieldName="' + cqt.code + '" invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>*</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input type="text" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>')
	};
	...
	if (cqt.answerInput == "INTEGER") { //整数;type='tel'
		_iptli = $('<li fieldName="' + cqt.code + '" invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>*</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input astype="integer" type="number" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>');
	};
	if (cqt.answerInput == "DOUBLE") { //小数;type='number'
		_iptli = $('<li fieldName="' + cqt.code + '" invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>*</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input type="number" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>');
	};
	if (cqt.answerInput == "TELEPHONE") { //电话;type='number'
		_iptli = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>*</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right phone"><input type="number" placeholder="区号"/><b>-</b><input type="number" placeholder="电话号码"/><b>-</b><input type="number" placeholder="分机号"/></div></li>');
	}
...
function left(d) { //CHECKBOXMULT类型;_id:给div设的id;cqt:大json;
	if (d.length != 0) {
			var _idsp = d.code.split("."); //分割code
			console.log(_idsp )
			var _id = ""; //给div设id
			for (var i = 0; i < _idsp.length; i++) {
				_id += _idsp[i]; //合并code
		...
		var cp = $("<div modelname='question' class='problem' id='"+_id+"'><div class='pb'><p class='view-title01' data='"+d.code+"' onclick='bjQuestion(this)'>" + d.name + "<em>编辑</em></div><ul></ul></div>");
		$("section").append(cp);
...
function _input(v) { //INPUT;v:大json;
	if (v.length != 0) {
			var _idsp = v.code.split("."); //分割code
			console.log(_idsp )
			var _id = ""; //给div设id
	...
	if (v.answerInput == "INTEGER") { //整数;type='tel'
		_iptli = $('<li invesT="' + v.isLabel + '" require="' + v.require + '" data="' + v.code + '" checkMark="' + v.checkMark + '" limitLength="' + v.limitLength + '"><div class="left"><span>*</span><i title="' + v.shortName + '">' + v.shortName + '</i></div><div class="right"><input astype="integer" type="number" placeholder="请输入' + v.shortName + '" /><b class="yuan">' + _cut + '</b></div></li>');
	};

	if (v.answerInput == "DOUBLE") { //小数;type='number'
		_iptli = $('<li invesT="' + v.isLabel + '" require="' + v.require + '" data="' + v.code + '" checkMark="' + v.checkMark + '" limitLength="' + v.limitLength + '"><div class="left"><span>*</span><i title="' + v.shortName + '">' + v.shortName + '</i></div><div class="right"><input type="number" placeholder="请输入' + v.shortName + '" /><b class="yuan">' + _cut + '</b></div></li>');
	};
	if (v.answerInput == "STRING") { //文本;type='text'
		_iptli = $('<li invesT="' + v.isLabel + '" require="' + v.require + '" data="' + v.code + '" checkMark="' + v.checkMark + '" limitLength="' + v.limitLength + '"><div class="left"><span>*</span><i title="' + v.shortName + '">' + v.shortName + '</i></div><div class="right"><input type="text" placeholder="请输入' + v.shortName + '" /><b class="yuan"></b>' + _cut + '</div></li>')
	};
...
function _tb(id, b, _id, vCode) { //id(du):_id下面的ul,b:大json,_id:code拼接字符串,vCode:动态显示的问题;
	...
	if (b.questionTableAnswerMap != null) { //判断json有没有传答案过来，没有则为null
		$('<div class="problem show" modelname="questionShow"><p>请完善' + b.shortName + '</p><div class="cy"></div><ul></ul></div>').insertAfter(id.parent());
									...
									for (var l = 0; l < t_n.length; l++) { //循环截取的数组
										if (t_n[l].substring(0, t_n[l].indexOf(":")) == tsj[i][j]["value"]) { //判断guid和value是否相等
											if (t_n[l].substring(t_n[l].indexOf(":") + 1).length > 4) { //判断中文长度是否大于4,大于4就截取前4位
												$('<span>' + t_n[l].substring(t_n[l].indexOf(":") + 1).substr(0, 4) + '</span>').appendTo(id.parent().siblings(".show").children(".cy"))
											} else {
												$('<span>' + t_n[l].substring(t_n[l].indexOf(":") + 1) + '</span>').appendTo(id.parent().siblings(".show").children(".cy"))
											}
		...
		$('<div class="problem show" modelname="questionShow"><p>请完善' + b.shortName + '</p><div class="cy"><span>页签</span></div><ul></ul></div>').insertAfter(id.parent());
		$(".cy span:first-child").addClass("span_bg"); //第一个span添加类名
		...
		for (var i = 0; i < l_i.length; i++) { //循环所有的li
			if (l_i.eq(i).attr("invest") == 1) { //判断isLbel为1的li
				...
				if (l_i.eq(i).find(".right span").length == 1) { //判断类型;span类型
					...
					s_t = l_i.eq(i).find(".left i").text();
				} else if (l_i.eq(i).find(".left input").length == 1) { //input类型
					s_l = l_i.eq(i).find(".left i").text();
					s_t = l_i.eq(i).find(".left input").val();
			...
			for (var p = 0; p < tsj[$(this).index() + 1].length; p++) { //循环当前选中列的数据
				if (_ids == tsj[$(this).index() + 1][p].investigateTableCode) { //判断code名和li的code名是否相等
					...
					if (id.parent().next(".show").find("ul li").eq(i).find(".right span").length == 1) { //判断类型;span类型
							...
							if (_lcd != undefined) { //判断问题是否为code类型的
								...
								for (var y = 0; y < _tns.length; y++) { //循环截取出来的数组
									if (tsj[$(this).index() + 1][p].value.match(_tns[y].substring(0, _tns[y].indexOf(":")))) { //判断guid和value是否相等
										id.parent().next(".show").find("ul li").eq(i).find(".right span").text(_tns[y].substring(_tns[y].indexOf(":") + 1)); //把中文到span上
						...
						if (tsj[$(this).index() + 1][p].value != null) { //判断答案不为空
							id.parent().next(".show").find("ul li").eq(i).find(".right input").val(tsj[$(this).index() + 1][p].value); //把值赋到input上
						}
						if (tsj[$(this).index() + 1][p].value != null) {
						...
						if (tsj[$(this).index() + 1][p].value != null) { //判断答案不为空
							id.parent().next(".show").find("ul li").eq(i).find(".textCons textarea").val(tsj[$(this).index() + 1][p].value); //给textarea赋值
	...
	id.parent().next(".show").find("p .jian").unbind("click").on("click", function() {
		...
		if (num > 1) { //num大于1的话，添加删除在页签上
			for (var i = 0; i < id.parent().next(".show").children(".cy").children().length; i++) {
				$("<img src='img/close_icon.png'/>").appendTo(id.parent().next(".show").children(".cy").children().eq(i));
			}
		};
						...
						if (newC[i].investigateTableCode == du.children().eq(j).attr("data")) { //判断li的code和后台传过来的答案的code是否相同
							...
							if (du.children().eq(j).find(".right span").length == 1) { //判断li是什么样的类型，span类型
									...
									if (du.children().eq(j).attr("_cd") != undefined) { //判断类型是否是code(弹窗类型)
										...
										for (var q = 0; q < _qa.length; q++) { //循环切割开来的数组
											if (_qa[q].substring(0, _qa[q].indexOf(":")) == newC[i].value) { //判断guid是否相等
												du.children().eq(j).find(".right span").text(_qa[q].substring(_qa[q].indexOf(":") + 1)); //把guid所对应的中文显示到页面上
								...
								if (newC[i].value != "" && newC[i].value != null) {
									du.children().eq(j).find(".right input").val(newC[i].value); //把值附到input上
									du.children().eq(j).find(".left i").css({
										"color": "#a09fa4"
									});
...
function crd(cqt, _id, du, _idj) { //创建页面cqt:c.questionTableList[i];
	...
	if (cqt.type == "STRING") { //文本框
		...
		if (cqt.require) { //require:是否必填,data:问题code,checkMark:校验类型,limitLength:文本长度
			_crdLi = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>*</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input type="text" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>')
			_crdLi.appendTo(du);
		} else {
			_crdLi = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>&nbsp;&nbsp;</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input type="text" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>')
			_crdLi.appendTo(du)
	...
	if (cqt.type == "DATE") { //日期
		...
		var dateID = 'date' + _idv;
		if (cqt.require) { //是否必填
			_crdLi = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>*</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input id="' + dateID + '" type="text" readonly name="input_date" placeholder="日期选择" data-lcalendar="1916-01-11,2116-05-29"/></div></li>');
			_crdLi.appendTo(du);
		} else {
			_crdLi = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>&nbsp;&nbsp;</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input id="' + dateID + '" type="text" readonly name="input_date" placeholder="日期选择" data-lcalendar="1916-01-11,2116-05-29" /></div></li>');
			_crdLi.appendTo(du);
	...
	if (cqt.type == "INTEGER") { //整数
		...
		if (cqt.require) { //是否必填
			_crdLi = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>*</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input kj type="number" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>');
			_crdLi.appendTo(du);
		} else {
			_crdLi = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>&nbsp;&nbsp;</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input astype="integer" type="number" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>');
			_crdLi.appendTo(du);
		}
	};
	...
	if (cqt.type == "DOUBLE") { //小数
		...
		if (cqt.require) { //是否必填
			_crdLi = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>*</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input type="number" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>');
			_crdLi.appendTo(du);
		} else {
			_crdLi = $('<li invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '"><div class="left"><span>&nbsp;&nbsp;</span><i title="' + cqt.name + '">' + cqt.name + '</i></div><div class="right"><input type="number" placeholder="请输入' + cqt.name + '"/><b class="yuan">' + _cut + '</b></div></li>');
			_crdLi.appendTo(du);
	...
	if (cqt.type == "TEXTAREA") { //textarea
		...
		if (cqt.require) {
			$('<li class="textareaStyle" invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '">' +
				...
				'</div>' +
				'<div class="textCons right">' +
				'<textarea style="font-size:0.4rem;height:2rem;width:97%;padding:0.1rem;border:0.05rem #e6e6eb solid;border-radius:0.1rem;"></textarea>' +
				'</div>' +
				'</li>').appendTo(du);
		} else {
			$('<li class="textareaStyle" invesT="' + cqt.isLabel + '" require="' + cqt.require + '" data="' + cqt.code + '" checkMark="' + cqt.checkMark + '" limitLength="' + cqt.limitLength + '">' +
...
function bjQuestion(id){
		var questionId=$(id).attr('data');
//		alert(questionId);
 		AndroidJs.editQuestionClick(questionId);
}
