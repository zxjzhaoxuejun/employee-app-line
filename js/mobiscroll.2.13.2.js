        ...
        that.show = function (prevAnim, prevFocus) {
            ...
            setTimeout(function () {
                ...
                $('body').append('<div class="clear-text">清除</div>');
                 $('.clear-text').animate({bottom:"3.85rem",opacity:'1'},50);
            }, 300);
...
//          
            if (multiple) {
            	...
            	var dayNow=Months.getDate();
            	console.log(dayNow)
            	if(dayNow>=30){
//          	alert(val)
                    /****2017-07-31修改***/
	            	if(v>20){
	            		onEndVal=onEndVal+100;
	            	}else{
	            		console.log("Val:"+val)
	            		console.log("onEndVal:"+onEndVal);
	            		console.log("v:"+v);
	            		
		            	if(onEndVal<=v){
		            		v=Math.round(onEndVal);
		            	}else{
		            		v=v;
		            	}
		            	cell= cells.eq(v);
...
//                      res.v=res.v-1;
                ...
                //2017-07-31修改
            var selectYearVal=$('.dw-sel .dw-i').eq(0).text();
            var selectMonthVal=$('.dw-sel .dw-i').eq(1).text();
            that._valueText =selectYearVal+"-"+selectMonthVal;
        ...
        function setValue(fill, change, time, noscroll, temp) {
            ...
            var selectYearVal=$('.dw-sel .dw-i').eq(0).text();
            var selectMonthVal=$('.dw-sel .dw-i').eq(1).text();
            var selectV=selectYearVal+"-"+selectMonthVal;
            if(selectV=="-"){
            	selectV='';
            }
             valueText=that._valueText =selectV
