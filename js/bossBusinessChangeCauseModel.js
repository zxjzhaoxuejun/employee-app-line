...
function loadBossBusinessChangeCause(sjson) {
    ...
    for (var j = 0; j < subList.length; j++) {
        ...
        if (subList[j].type == "TABLE") {
            ...
            for (var i = 0; i < subList[j].questionDetailList.length; i++) {
                ...
                switch (Types) {
                    case "DOUBLE":
                        create_Textarea(subList[j].questionDetailList[i], sUl_table, oSction, sDiv_table, subList[j].objName,subList[j].name);
                        break;
                    case "TEXTAREA":
